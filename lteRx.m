function [rxBits, rxCW, crc, rxgrid, eqGrid, enbRx]= lteRx(rxWaveform, PDSCH)
...
while (size(rxgrid,2) > 0)
    ...
    for i = 1:numel(alldci)
        ...
        [pdsch, trblklen] = hPDSCHConfiguration(enbRx, dci, pdcch.RNTI);
        ...
        if ~isempty(pdsch)
            ...
            % Get PDSCH indices
            [pdschIndices,pdschIndicesInfo] = ltePDSCHIndices(enbRx, pdsch, pdsch.PRBSet);
            [pdschRx, pdschHest] = lteExtractResources(pdschIndices, rxsubframe, hest);
            % Decode PDSCH 
            [dlschBits,pdschSymbols] = ltePDSCHDecode(enbRx, pdsch, pdschRx, pdschHest, nest);
            % Decode DL-SCH with soft buffer input/output for HARQ combining
            if ~isempty(decState)
                fprintf('Recombining with previous transmission.\n\n');
            end        
            [sib1, crc, decState] = lteDLSCHDecode(enbRx, pdsch, trblklen, dlschBits, decState);

            % Compute PDSCH EVM
            recoded = lteDLSCH(enbRx, pdsch, pdschIndicesInfo.G, sib1);
            remod = ltePDSCH(enbRx, pdsch, recoded);
            [~,refSymbols] = ltePDSCHDecode(enbRx, pdsch, remod);
...
% [estChannelGrid,noiseEst] = lteDLChannelEstimate(enbRx,cec, rxSubframe);
...
[rxEncodedBits, rxCW] = ltePDSCHDecode(enbRx,PDSCH,rxSubframe,estChannelGrid,noiseEst);

% Decode DownLink Shared Channel (DL-SCH)
PDSCH.NTurboDecIts = 5;
if iscell(rxEncodedBits) && ~iscell(PDSCH.Modulation)
   [rxBits,crc, ~] = lteDLSCHDecode(enbRx,PDSCH,PDSCH.TrBlkSize,rxEncodedBits{1});
else
   [rxBits,crc, ~] = lteDLSCHDecode(enbRx,PDSCH,PDSCH.TrBlkSize,rxEncodedBits);
end
